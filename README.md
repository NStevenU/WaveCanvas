...
## 📦 주요 부품 리스트 (Bill of Materials)

WaveCanvas 본체 및 외장 스피커 모듈 자작에 사용된 핵심 부품 목록입니다:

### 1. 🎛️ WaveCanvas 본체 부품
| 부품 구분 | 사용 부품 명칭 | 규격 및 권장 사양 | 비고 |
| :--- | :--- | :--- | :--- |
| **메인 MCU** | **ESP32-S3 DevKit** | **N16R8** (16MB Flash + 8MB Octal PSRAM) | 32보이스 및 SF2 사운드폰트 로딩 필수 |
| **I2S DAC** | **PCM5102A 모듈** | 32-bit / 384kHz I2S DAC 보드 (보라색 PCB) | 고음질 아날로그 라인아웃/헤드폰 출력 |
| **시리얼 통신** | **MAX3232 모듈** | RS-232 to TTL 변환 모듈 | 구형 DOS PC / 노트북 9핀 COM 포트 직결 |
| **디스플레이** | **0.96인치 OLED** | 128x64 I2C OLED (SSD1306, 4핀) | 크기만 맞으면 색상 상관없이 사용가능 |
| **입력 엔코더** | **EC11 로터리 엔코더** | 원형 PCB 모듈 타입 (푸시 스위치 내장) | 3D 프린팅 전면 베젤 홀 규격 일치 |
| **기판 & 배선** | **만능기판 & 핀헤더** | 표준 2.54mm 피치 만능기판(Perfboard) | 모듈 탈부착용 암/수 핀헤더 소켓 |

---

### 2. 🔊 외장 도킹 스피커 (Nano SPK) 부품
| 부품 구분 | 사용 부품 명칭 | 규격 및 권장 사양 | 비고 |
| :--- | :--- | :--- | :--- |
| **스피커 유닛** | **28mm 풀레인지 유닛** | **4Ω 3W (FUET 풀레인지)** | 인클로저 규격에 딱 맞는 28mm 네오디뮴 유닛 |
| **오디오 앰프** | **5V 미니 앰프 모듈** | 5V 구동 초소형 앰프 (PAM8403 등) | 인클로저 내부에 들어가는 소형 앰프 |
| **음향 댐퍼** | **마스크 부직포** | 일회용 마스크 내부 부직포 1겹 | `SPK Port.stl` 덕트 배출구 저역 음향 필터용 |
| **도킹 커넥터** | **6핀 커넥터/헤더** | 2.54mm 피치 6핀 커넥터 | 5V, GND, SPK_DET, L-Out, AGND 도킹 연결 |
...
본 프로젝트의 **전체 컨셉 기획, 시스템 및 회로 설계, 전체적인 디자인 및 3D모델링, 하드웨어 제작 및 실기 테스트는 제작자가 총괄**하였으며, 개발 과정에서 코딩 작성 보조 및 교차 검증 도구로 아래의 AI 모델들을 페어 프로그래밍 어시스턴트로 활용했습니다:
* **Gemini 3.7 Flash (High)**: 개발자의 아키텍처 및 요구사항에 따른 소스코드 작성, 기능 구현 및 코드 리팩토링 보조
* **Claude 4.6 Opus (Thinking)**: 정밀 코드 전수 검사, 엣지 케이스 분석 및 코드 크로스 체크(더블 체크)
* **GitHub Copilot Auto**: 정밀 코드 전수 검사, 엣지 케이스 분석 및 코드 크로스 체크(더블 체크)
