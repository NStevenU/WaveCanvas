...
static String generateHTML(const String& tab, const String& lang, const String& selSsid = "") {
    ...
    html += "body { background-color: #ffffff; color: #000000; font-family: Tahoma, 'MS Sans Serif', Dotum, Gulim, sans-serif; font-size: 12px; margin: 10px; }\n";
    ...
    html += ".btn98 { background-color: #d4d0c8; border: 2px solid; border-color: #ffffff #808080 #808080 #ffffff; padding: 3px 8px; font-size: 12px; color: #000000; text-decoration: none; cursor: pointer; display: inline-block; vertical-align: middle; }\n";
    ...
    // 가상 피아노 건반 스타일 (레트로 직각 3D 베벨 디자인)
    html += ".piano-box { background: #222222; padding: 12px; border: 2px solid; border-color: #808080 #dfdfdf #dfdfdf #808080; display: inline-block; user-select: none; -webkit-user-select: none; }\n";
    html += ".piano-kb { position: relative; height: 145px; display: inline-block; font-size: 0; white-space: nowrap; line-height: 0; }\n";
    html += ".w-key { display: inline-block; width: 34px; height: 140px; background: #ffffff; border: 1px solid #7f7f7f; border-bottom: 5px solid #a0a0a0; cursor: pointer; vertical-align: top; position: relative; box-sizing: border-box; text-align: center; line-height: normal; }\n";
    html += ".w-key.active, .w-key:active { background: #d0e4ff; border-bottom: 2px solid #777777; transform: translateY(3px); }\n";
    html += ".w-lbl { position: absolute; bottom: 4px; left: 0; width: 100%; font-size: 10px; font-weight: bold; color: #333333; pointer-events: none; }\n";
    html += ".w-hk { position: absolute; bottom: 18px; left: 0; width: 100%; font-size: 9px; color: #888888; pointer-events: none; }\n";
    html += ".b-key { position: absolute; top: 0px; width: 22px; height: 85px; background: #111111; border: 1px solid #000000; border-bottom: 4px solid #333333; cursor: pointer; z-index: 2; box-sizing: border-box; text-align: center; line-height: normal; }\n";
    html += ".b-key.active, .b-key:active { background: #3a3a3a; border-bottom: 2px solid #000000; transform: translateY(2px); }\n";
    html += ".b-lbl { position: absolute; bottom: 3px; left: 0; width: 100%; font-size: 8px; font-weight: bold; color: #ffffff; pointer-events: none; }\n";
    html += ".b-hk { position: absolute; bottom: 14px; left: 0; width: 100%; font-size: 8px; color: #ffcc00; pointer-events: none; }\n";
    html += "</style>\n";
    ...
    // IE 4.0 ~ 모던 브라우저 공용 비동기 요청 래퍼 (Fetch -> XHR -> ActiveX -> Image Beacon)
    html += "function sendApi(url) {\n";
    html += "  if (window.fetch) { fetch(url); }\n";
    html += "  else if (window.XMLHttpRequest) {\n";
    html += "    var xhr = new XMLHttpRequest();\n";
    html += "    xhr.open('GET', url, true); xhr.send();\n";
    html += "  } else if (window.ActiveXObject) {\n";
    html += "    try {\n";
    html += "      var xhr = new ActiveXObject('Microsoft.XMLHTTP');\n";
    html += "      xhr.open('GET', url, true); xhr.send();\n";
    html += "    } catch(e) { new Image().src = url; }\n";
    html += "  } else {\n";
    html += "    new Image().src = url;\n";
    html += "  }\n";
    html += "}\n";

    html += "function updateVol(val) {\n";
    ...
    // 피아노 3회 연속 클릭 시 히든 탭 해금 이스터에그 (SessionStorage / Cookie 호환)
    ...
    html += "function setKeyClass(el, add) {\n";
    html += "  if (!el) return;\n";
    html += "  if (el.classList) { if (add) el.classList.add('active'); else el.classList.remove('active'); }\n";
    html += "  else { el.className = add ? (el.className + ' active') : el.className.replace(/\\bactive\\b/g, ''); }\n";
    html += "}\n";
    ...
    html += "  setKeyClass(getEl('k_' + note), true);\n";
    ...
    html += "  setKeyClass(getEl('k_' + note), false);\n";
    ...
    html += "  if (keyMap[k] !== undefined && !e.repeat) {\n";
    ...
    html += "  if (tgt && (tgt.tagName === 'INPUT' || tgt.tagName === 'SELECT')) return;\n";
    html += "  var k = (e.key ? e.key : String.fromCharCode(e.keyCode)).toLowerCase();\n";
    html += "  if (keyMap[k] !== undefined) {\n";
    html += "    pianoNoteOff(keyMap[k]);\n";
    html += "  }\n";
    html += "}\n";

    // 이벤트 등록 (W3C addEventListener -> IE attachEvent -> DOM Level 0 onkeydown/onload)
    html += "if (window.addEventListener) {\n";
    html += "  window.addEventListener('keydown', onKeyDn);\n";
    html += "  window.addEventListener('keyup', onKeyUp);\n";
    html += "} else if (window.attachEvent) {\n";
    html += "  document.attachEvent('onkeydown', onKeyDn);\n";
    html += "  document.attachEvent('onkeyup', onKeyUp);\n";
    html += "} else {\n";
    ...
    html += "  if (unlocked) {\n";
    html += "    var pt = getEl('tabPiano'); if (pt) pt.style.display = 'inline-block';\n";
    html += "  }\n";
    html += "}\n";
    html += "if (window.addEventListener) {\n";
    html += "  window.addEventListener('DOMContentLoaded', initPianoTab);\n";
    html += "} else {\n";
    html += "  var oldOnload = window.onload;\n";
    html += "  window.onload = function() { if (oldOnload) oldOnload(); initPianoTab(); };\n";
    html += "}\n";
    html += "</script>\n</head>\n<body>\n";
    ...
    html += "<tr>\n";
    html += "<td width=\"190\" valign=\"middle\">\n";
    html += "<a href=\"/?tab=player&lang=" + lang + "\"><img src=\"/logo.png\" width=\"182\" height=\"42\" border=\"0\" alt=\"WaveCanvas Logo\"></a>\n";
    html += "</td>\n";
    html += "<td valign=\"middle\" style=\"padding-left: 10px;\">\n";
    html += "<a href=\"/?tab=player&lang=" + lang + "\" class=\"" + (activeTab == "player" ? "tab-active" : "tab-inactive") + "\"><img src=\"/icon/music.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "MIDI 플레이어" : "MIDI Player") + "</a> ";
    if (g_font_mgmt_enabled) {
        html += "<a href=\"/?tab=fonts&lang=" + lang + "\" class=\"" + (activeTab == "fonts" ? "tab-active" : "tab-inactive") + "\"><img src=\"/icon/piano.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "사운드폰트" : "SoundFonts") + "</a> ";
    }
    html += "<a href=\"/?tab=wifi&lang=" + lang + "\" class=\"" + (activeTab == "wifi" ? "tab-active" : "tab-inactive") + "\"><img src=\"/icon/wifi.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "와이파이 설정" : "Wi-Fi Setup") + "</a> ";
    html += "<a href=\"/?tab=settings&lang=" + lang + "\" class=\"" + (activeTab == "settings" ? "tab-active" : "tab-inactive") + "\"><img src=\"/icon/setup.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "시스템 설정" : "Settings") + "</a> ";
    html += "<a href=\"/?tab=piano&lang=" + lang + "\" id=\"tabPiano\" class=\"" + (activeTab == "piano" ? "tab-active" : "tab-inactive") + "\" style=\"" + (activeTab == "piano" ? "" : "display:none;") + "\"><img src=\"/icon/piano.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "가상 피아노" : "Virtual Piano") + "</a>\n";
    html += "</td>\n";
    html += "<td align=\"right\" valign=\"middle\">\n";
    html += "<font size=\"1\">Language: </font>";
    html += "<a href=\"/?tab=" + activeTab + "&lang=ko\" class=\"btn98\" style=\"padding:2px 5px; font-size:11px;" + (isKo ? "font-weight:bold; background-color:#b0b0b0;" : "") + "\"><img src=\"/icon/korean.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">한국어</a> ";
    html += "<a href=\"/?tab=" + activeTab + "&lang=en\" class=\"btn98\" style=\"padding:2px 5px; font-size:11px;" + (!isKo ? "font-weight:bold; background-color:#b0b0b0;" : "") + "\"><img src=\"/icon/english.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">English</a>\n";
    html += "</td>\n";
    html += "</tr>\n";
    html += "</table>\n<br>\n";
    ...
    if (activeTab == "player") {
        ...
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/music.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "표준 MIDI 파일 재생기 (SMF)" : "Standard MIDI File Player") + "</td></tr>\n";
        ...
        html += "<tr><td width=\"150\"><b>" + String(isKo ? "MIDI 파일 선택 (.mid):" : "Select .MID File:") + "</b></td>";
        html += "<td><input type=\"file\" name=\"file\"></td>";
        html += "<td><button type=\"submit\" class=\"btn98\" style=\"font-weight:bold;\"><img src=\"/icon/upload.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "업로드 및 재생" : "Upload & Play") + "</button></td></tr>\n";
        html += "</table>\n</form>\n<hr size=\"1\" color=\"#cccccc\">\n";

        html += "<table width=\"100%\" cellpadding=\"4\" cellspacing=\"0\">\n";
        bool isGameBgm = (MIDISequencer::isLoopEnabled() || DisplayUI::getMode() == SCREEN_GAME_RUNNING || DisplayUI::getMode() == SCREEN_MENU_GAMES);
        String trackTitle = isGameBgm ? (isKo ? "🕹️ 아케이드 게임 실행 중 (웹 제어 비활성화)" : "🕹️ Arcade Game Active (Web Control Disabled)") : String(MIDISequencer::getCurrentSongName());
        ...
        if (!isGameBgm) {
            html += "<a href=\"/action?cmd=play&lang=" + lang + "\" onclick=\"sendApi('/action?cmd=play&ajax=1'); updatePlayerStatus('PLAYING'); return false;\" class=\"btn98\" style=\"font-weight:bold;\"><img src=\"/icon/play.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "재생" : "Play") + "</a> ";
            html += "<a href=\"/action?cmd=pause&lang=" + lang + "\" onclick=\"sendApi('/action?cmd=pause&ajax=1'); updatePlayerStatus('PAUSED'); return false;\" class=\"btn98\"><img src=\"/icon/pause.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "일시정지" : "Pause") + "</a> ";
            html += "<a href=\"/action?cmd=stop&lang=" + lang + "\" onclick=\"sendApi('/action?cmd=stop&ajax=1'); updatePlayerStatus('STOPPED'); return false;\" class=\"btn98\" style=\"color:#800000;\"><img src=\"/icon/stop.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "정지" : "Stop") + "</a>\n";
        } else {
        ...
        html += "<td width=\"100\"><img src=\"/icon/speaker.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\"><b>" + String(isKo ? "마스터 볼륨:" : "Volume:") + "</b></td>";
        html += "<td><input type=\"range\" min=\"0\" max=\"100\" value=\"" + String(AudioEngine::getMasterVolume()) + "\" id=\"volRange\" oninput=\"updateVol(this.value)\" style=\"width:180px; vertical-align:middle; cursor:pointer;\"> ";
        html += "<span id=\"volVal\" style=\"font-weight:bold; color:#000080; display:inline-block; width:45px; text-align:right; font-family:monospace;\">" + String(AudioEngine::getMasterVolume()) + "%</span></td>";
        html += "</tr></table>\n</td></tr>\n";
        ...
        html += "<button type=\"button\" class=\"btn98\" onclick=\"onPianoTestClick()\" style=\"margin-right:5px; font-weight:bold;\">" + String(isKo ? "피아노 C화음 (도미솔도)" : "Piano C Chord") + "</button> ";
        html += "<button type=\"button\" class=\"btn98\" onclick=\"playTest('guitar')\" style=\"margin-right:5px;\">" + String(isKo ? "기타 아르페지오" : "Guitar Arpeggio") + "</button> ";
        html += "<button type=\"button\" class=\"btn98\" onclick=\"playTest('drum')\" style=\"margin-right:5px;\">" + String(isKo ? "드럼 키트 (Kick/Snare)" : "Drums Kit") + "</button> ";
        html += "<button type=\"button\" class=\"btn98\" onclick=\"playTest('stereo')\">" + String(isKo ? "스테레오 테스트 (Left -> Right -> All)" : "Stereo Test (Left -> Right -> All)") + "</button>\n";
        html += "</div>\n";
        html += "</td></tr>\n";
        ...
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/folder.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "기기 내 저장된 MIDI 라이브러리" : "Stored MIDI Library") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        html += "<div class=\"inset-box\" style=\"height: 120px; overflow: auto;\">\n";
        html += "<table width=\"100%\" cellpadding=\"3\" cellspacing=\"0\">\n";
        html += "<tr bgcolor=\"#e0e0e0\"><th align=\"left\">" + String(isKo ? "파일명" : "File Name") + "</th><th width=\"80\">" + String(isKo ? "크기" : "Size") + "</th><th width=\"120\">" + String(isKo ? "관리" : "Action") + "</th></tr>\n";
        ...
        if (root && root.isDirectory()) {
            ...
            while (f) {
                ...
                if (name.endsWith(".mid") || name.endsWith(".MID")) {
                    ...
                    html += "<td><b>" + name + "</b></td><td align=\"center\">" + String((float)f.size() / 1024.0f, 1) + " KB</td>";
                    html += "<td align=\"center\">";
                    html += "<a href=\"/action?cmd=play_midi&file=" + urlEncode(name) + "&lang=" + lang + "\" onclick=\"sendApi('/action?cmd=play_midi&file=" + urlEncode(name) + "&ajax=1'); updatePlayingSong('" + name + "'); return false;\" class=\"btn98\" style=\"padding:1px 5px;\"><img src=\"/icon/play.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">Play</a> ";
                    html += "<a href=\"/action?cmd=delete_midi&file=" + urlEncode(name) + "&lang=" + lang + "\" class=\"btn98\" style=\"color:#800000; padding:1px 5px;\">" + String(isKo ? "삭제" : "Del") + "</a>";
                    html += "</td></tr>\n";
                    midiCount++;
                }
                f = root.openNextFile();
    ...
    // ---------------- [탭 2: 사운드폰트 관리] ----------------
        ...
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/folder.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "플래시 메모리 용량 현황 (LittleFS)" : "Flash Storage Status (LittleFS)") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        ...
        // 사운드폰트 업로드 박스
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/upload.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "사운드폰트 (.SF2) 파일 업로드" : "Install SoundFont (.SF2)") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        html += "<form action=\"/upload_font?lang=\" + lang + \"\" method=\"POST\" enctype=\"multipart/form-data\">\n";
        html += "<table width=\"100%\" cellpadding=\"4\" cellspacing=\"0\">\n";
        html += "<tr><td width=\"150\"><b>" + String(isKo ? "사운드폰트 선택:" : "Select SoundFont:") + "</b></td>";
        html += "<td><input type=\"file\" name=\"file\" accept=\".sf2,.SF2\"></td>";
        html += "<td><button type=\"submit\" class=\"btn98\" style=\"font-weight:bold;\"><img src=\"/icon/upload.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "플래시 메모리에 저장" : "Upload to Flash") + "</button></td></tr>\n";
        html += "</table>\n</form>\n";
        html += "</td></tr></table>\n";

        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/piano.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "플래시 메모리 사운드폰트 목록" : "SoundFonts in Flash Memory") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
    ...
    // ---------------- [탭 3: Wi-Fi 설정] ----------------
        ...
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/wifi.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "주변 무선 네트워크 (2.4GHz Wi-Fi)" : "Nearby Wireless Networks") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        ...
        if (WiFiManager::isScanning()) {
            ...
            html += "<td align=\"right\"><a href=\"/action?cmd=scan_wifi&lang=" + lang + "\" class=\"btn98\" style=\"font-weight:bold;\"><img src=\"/icon/wifi.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "Wi-Fi 검색" : "Scan Networks") + "</a></td></tr>\n";
        }
        html += "</table>\n";
        ...
        if (nets.size() > 0) {
            for (const auto& n : nets) {
                html += "<tr><td><a href=\"/?tab=wifi&sel_ssid=" + urlEncode(n.ssid) + "&lang=" + lang + "\" style=\"color:#000080;\"><img src=\"/icon/wifi.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\"><b>" + n.ssid + "</b></a></td>";
        ...
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/setup.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "Wi-Fi 공유기 접속 설정" : "Connect to Network") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        ...
        html += "<tr><td></td><td style=\"padding-top:6px;\"><button type=\"submit\" class=\"btn98\" style=\"font-weight:bold;\"><img src=\"/icon/setup.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "접속 및 NVRAM 영구 저장" : "Connect & Save to NVRAM") + "</button></td></tr>\n";
        html += "</table>\n</form>\n</td></tr></table>\n";
        ...
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/port.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "현재 연결된 네트워크 상세 정보" : "Active Network Details") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        html += "<table width=\"100%\" cellpadding=\"3\" cellspacing=\"0\">\n";
    ...
    // ---------------- [탭 4: 시스템 설정] ----------------
        ...
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/speaker.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "마스터 출력 볼륨 (Master Volume)" : "Master Output Volume") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        html += "<table width=\"100%\" cellpadding=\"4\" cellspacing=\"0\">\n";
        html += "<tr><td width=\"120\"><b>" + String(isKo ? "실시간 볼륨 조절:" : "Live Volume:") + "</b></td>";
        html += "<td><input type=\"range\" min=\"0\" max=\"100\" value=\"" + String(AudioEngine::getMasterVolume()) + "\" id=\"volRange2\" oninput=\"updateVol(this.value)\" style=\"width:220px; vertical-align:middle; cursor:pointer;\"> ";
        html += "<span id=\"volVal2\" style=\"font-weight:bold; color:#000080; display:inline-block; width:45px; text-align:right; font-family:monospace;\">" + String(AudioEngine::getMasterVolume()) + "%</span></td></tr>\n";
        html += "</table>\n";
        html += "</td></tr></table>\n";

        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/port.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "시리얼 MIDI 통신 포트 (UART2)" : "Serial MIDI Port (UART2)") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        ...
        // ---------------- [소리 출력 설정 (오디오 모드)] ----------------
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/speaker.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "소리 출력 설정 (Audio Output Mode)" : "Audio Output Mode") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        html += "<form action=\"/action\" method=\"GET\">\n";
        html += "<input type=\"hidden\" name=\"tab\" value=\"settings\">\n";
        ...
        // ---------------- [LED 상태표시등 설정] ----------------
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/lamp.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "LED 상태표시등" : "LED Status Indicator") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        html += "<form action=\"/action\" method=\"GET\">\n";
        html += "<input type=\"hidden\" name=\"tab\" value=\"settings\">\n";
        ...
        // ---------------- [시간 및 NTP 동기화 설정] ----------------
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/setup.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "시스템 시간 및 NTP 동기화 설정" : "System Time & NTP Configuration") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        html += "<form action=\"/action\" method=\"GET\">\n";
        html += "<input type=\"hidden\" name=\"tab\" value=\"settings\">\n";
        ...
        // ---------------- [사운드폰트 고급 관리 모드 설정] ----------------
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/piano.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "사운드폰트 고급 관리 모드 (SoundFont Advanced Mode)" : "SoundFont Advanced Mode") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        html += "<form action=\"/action\" method=\"GET\">\n";
        html += "<input type=\"hidden\" name=\"tab\" value=\"settings\">\n";
        ...
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/warning.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "하드웨어 제어 및 진단" : "Diagnostics") + "</td></tr>\n";
        html += "<tr><td style=\"padding: 10px;\">\n";
        html += "<a href=\"/action?cmd=panic&tab=settings&lang=" + lang + "\" class=\"btn98\" style=\"color:#800000; font-weight:bold;\"><img src=\"/icon/warning.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "MIDI 패닉 (강제 무음 리셋 / All Notes Off)" : "MIDI Panic (All Sound Off)") + "</a>\n";
        html += "</td></tr></table>\n";
    }
    ...
    // ---------------- [탭 5: 히든 가상 피아노 건반 (Easter Egg)] ----------------
        ...
        html += "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"section-table\">\n";
        html += "<tr><td class=\"section-hdr\"><img src=\"/icon/piano.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\">" + String(isKo ? "히든 메뉴! 가상 피아노" : "Hidden Menu! Virtual Piano") + "</td></tr>\n";
        ...
        html += "<div style=\"margin-bottom:12px; color:#333333; font-size:11px;\">" + 
                String(isKo ? "<img src=\"/icon/lamp.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\"><b>키보드 연주 지원:</b> PC 키보드 <code>A S D F G H J K L ; '</code> 및 <code>W E T Y U O P ]</code> 키를 누르면 실시간으로 건반이 연주됩니다! (마우스 클릭/터치 가능)"
                            : "<img src=\"/icon/lamp.png\" width=\"16\" height=\"16\" class=\"ic\" border=\"0\" alt=\"\"><b>Keyboard Supported:</b> Play notes using keys <code>A S D F G H J K L ; '</code> and <code>W E T Y U O P ]</code>! Mouse and touch also supported.") + 
                "</div>\n";
        ...
        for (int i = 0; i < 14; i++) {
            html += "<div class=\"w-key\" id=\"k_" + String(wMidi[i]) + "\" onmousedown=\"pianoNoteOn(" + String(wMidi[i]) + ")\" onmouseup=\"pianoNoteOff(" + String(wMidi[i]) + ")\" onmouseleave=\"pianoNoteOff(" + String(wMidi[i]) + ")\" ontouchstart=\"pianoNoteOn(" + String(wMidi[i]) + "); event.preventDefault();\" ontouchend=\"pianoNoteOff(" + String(wMidi[i]) + ")\">";
    ...
    html += "<td>Active Font: <b>" + String(AudioEngine::getCurrentFontName()) + "</b></td>\n";
    html += "<td align=\"center\">IP: <b>" + WiFiManager::getIPAddress() + "</b> (" + String(WiFiManager::getModeString()) + ")</td>\n";
    html += "<td align=\"right\">MIDI: <b>" + String(MIDIParser::getBaudRate()) + " bps</b></td>\n";
    html += "</tr></table>\n";

    // 4. 회사 저작권 표시 (Copyright)
    html += "<div align=\"center\" style=\"font-size:11px; color:#666666; margin-top:8px; margin-bottom:4px;\">\n";
...
void WebManager::begin() {
    ...
    // 1. 로고 이미지 라우트 (/logo.png) - Flash/PROGMEM에서 초고속 서빙
    server.on("/logo.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", LOGO_PNG_DATA, LOGO_PNG_SIZE);
    });

    // 2. 16x16 아이콘 이미지 라우트 등록 (/icon/...)
    server.on("/icon/music.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_MUSIC_PNG_DATA, ICON_MUSIC_PNG_SIZE);
    });
    server.on("/icon/piano.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_PIANO_PNG_DATA, ICON_PIANO_PNG_SIZE);
    });
    server.on("/icon/wifi.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_WIFI_PNG_DATA, ICON_WIFI_PNG_SIZE);
    });
    server.on("/icon/setup.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_SETUP_PNG_DATA, ICON_SETUP_PNG_SIZE);
    });
    server.on("/icon/korean.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_KOREAN_PNG_DATA, ICON_KOREAN_PNG_SIZE);
    });
    server.on("/icon/english.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_ENGLISH_PNG_DATA, ICON_ENGLISH_PNG_SIZE);
    });
    server.on("/icon/play.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_PLAY_PNG_DATA, ICON_PLAY_PNG_SIZE);
    });
    server.on("/icon/pause.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_PAUSE_PNG_DATA, ICON_PAUSE_PNG_SIZE);
    });
    server.on("/icon/stop.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_STOP_PNG_DATA, ICON_STOP_PNG_SIZE);
    });
    server.on("/icon/upload.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_UPLOAD_PNG_DATA, ICON_UPLOAD_PNG_SIZE);
    });
    server.on("/icon/folder.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_FOLDER_PNG_DATA, ICON_FOLDER_PNG_SIZE);
    });
    server.on("/icon/speaker.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_SPEAKER_PNG_DATA, ICON_SPEAKER_PNG_SIZE);
    });
    server.on("/icon/port.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_PORT_PNG_DATA, ICON_PORT_PNG_SIZE);
    });
    server.on("/icon/warning.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_WARNING_PNG_DATA, ICON_WARNING_PNG_SIZE);
    });
    server.on("/icon/lamp.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "image/png", ICON_LAMP_PNG_DATA, ICON_LAMP_PNG_SIZE);
    });
    ...
    // 3-2. 가상 피아노 실시간 Note On / Off / Program Change API
